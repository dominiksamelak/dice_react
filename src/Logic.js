...
export function CountingLogic({
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      if (scores[`player${playerString}Scores`][isConfirmedKey] === true) {
        continue;
      }
    ...
    if (currentPlayer === 1 && !scores.playerOneScores.isTripleConfirmed) {
      if (doubleScores) {
        scores.playerOneScores.triple =
          tripleScore !== 0 ? 2 * tripleScore : '---';
      } else {
